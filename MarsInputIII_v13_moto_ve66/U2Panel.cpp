...
void U2Panel::initPixmapBuffer(){
    ...
    pixbuffer=new QPixmap(240, 320);//(320,240);
}
	...
	if(selectMode){
		...
		if(key==4146){
			...
			if(menuShows){
				...
				plist += "aaaa";
				plist += "bbbb";
				ZSingleSelectDlg *dlg = new ZSingleSelectDlg("title", "test");
...
void U2Panel::keyRel(int key){
	...
	if(selectMode){//选词模式
		...
		}else if(key==4152){ // 左软键
			if(num_buf==""){
				menuShows=!menuShows;
			}
		}else if(key==4154){ // 右软键
			emit(accepted());
			//接受
		}
	}
...
void U2Panel::paintEvent(QPaintEvent* event){ //===================================
	...
	if(aboutDialogShows){
		...
		ptr.drawText(skin->about_dialog_x,yy,ctr("火星输入法："));
		yy+=20;
		...
		yy+=20;
		ptr.drawText(skin->about_dialog_x,yy,ctr("移植/修改：wanggjghost"));	
		yy+=20;
		ptr.drawText(skin->about_dialog_x+50,yy,ctr("(泪の单翼天使)"));	
		yy+=20;
		ptr.drawText(skin->about_dialog_x,yy,ctr("访问我们：www.591fan.com"));			
	}
	if(themeDialogShows){
		//背景
